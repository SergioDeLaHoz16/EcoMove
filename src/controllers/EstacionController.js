...
export class EstacionController {
  ...
  static crear(datosEstacion) {
    try {
      ...
      if (!('latitud' in datosEstacion) || datosEstacion.latitud === undefined || datosEstacion.latitud === null || datosEstacion.latitud === '') {
        datosEstacion.latitud = 0;
      }
      if (!('longitud' in datosEstacion) || datosEstacion.longitud === undefined || datosEstacion.longitud === null || datosEstacion.longitud === '') {
        datosEstacion.longitud = 0;
      }
      const estacion = new Estacion(datosEstacion);
      const errores = estacion.validar().filter(e => !e.toLowerCase().includes('latitud') && !e.toLowerCase().includes('longitud'));
  ...
  static obtenerTransportesDisponibles(estacionId) {
    ...
    import('../controllers/TransporteController.js').then(({ TransporteController }) => {
