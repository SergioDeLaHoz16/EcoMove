...
import { TransporteForm } from '../components/TransporteForm.jsx';
...
export function Transportes() {
  ...
  const getTransporteIcon = (tipo) => {
    switch (tipo) {
      case 'bicicleta': return <Bike className="h-5 w-5 text-green-600" />;
      case 'scooter': return <Zap className="h-5 w-5 text-orange-600" />;
      case 'auto_electrico': return <Car className="h-5 w-5 text-blue-600" />;
      default: return <Bike className="h-5 w-5 text-green-600" />;
    }
  };

  const transportesFiltrados = filtroTipo 
    ? transportes.filter(t => t.tipo === filtroTipo)
    : transportes;
              ...
              <div className="space-y-2 text-sm text-gray-600 mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`font-medium ${
                    transporte.estado === 'operativo' ? 'text-green-600' : 'text-red-600'
                  }`}>
                    {transporte.estado}
                  </span>
                </div>
              ...
              {transporte.kilometraje > 0 && (
                <div className="mt-3 pt-3 border-t border-gray-100">
                  <p className="text-xs text-gray-500">
                    Kilometraje: {transporte.kilometraje} km
                  </p>
                </div>
              )}
            </div>
